...
class Generator (nn.Module):
    ...
    def __init__(self, opt): # From the SRGAN paper: Our generator network has 16 identical (B = 16) residual blocks.
        super().__init__()

        # head - feature extractor
        feature_extractor = [nn.Conv2d(opt.in_channels, 64, 9, 1, 4), nn.PReLU()]

        # body - residual blocks
        res_body = []
        res_body.extend([ResidualBlock(64) for _ in range(opt.num_res_blocks)])
        res_body.append(nn.Conv2d(64, 64, 3, 1, 1))
        res_body.append(nn.BatchNorm2d(64))

        # tail - upscaler
        upscaler = []
        upscaler.extend([PixelShuffleBlock(64, 256, 2) for _ in range(2)])
        upscaler.append(nn.Conv2d(64, 3, 9, 1, 4))
